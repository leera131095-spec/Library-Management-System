...
export function AddBookPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* Header */}
      <div>
        <h1 className="font-serif text-3xl font-bold text-foreground">Add Book</h1>
        ...
        <div className="xl:col-span-2">
          <div className="bg-card rounded-[--radius-lg] p-6 fairytale-shadow border border-border/30">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="font-serif text-lg font-semibold text-foreground">New Book</h2>
            </div>

            <form onSubmit={handleAddBook} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label htmlFor="book-title" className="text-sm font-medium text-foreground">Book Title</label>
                <input
                  id="book-title"
                  type="text"
                  value={title}
                  ...
                  className="w-full px-4 py-2.5 rounded-[--radius-md] bg-input border border-border text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-ring/40 focus:border-primary transition-all text-sm"
                  required
                />
              </div>

              <div className="flex flex-col gap-1.5">
                <label htmlFor="book-author" className="text-sm font-medium text-foreground">Author</label>
                <input
                  id="book-author"
                  type="text"
                  value={author}
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="book-copies" className="text-sm font-medium text-foreground">Number of Copies</label>
                <input
                  id="book-copies"
                  type="number"
                  min="1"
        ...
        <div className="xl:col-span-3">
          <div className="bg-card rounded-[--radius-lg] fairytale-shadow border border-border/30 overflow-hidden">
            <div className="p-5 border-b border-border/30">
              <h3 className="font-serif text-lg font-semibold text-foreground">Library Collection</h3>
              <p className="text-xs text-muted-foreground mt-0.5">{books?.length ?? 0} books in catalog</p>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-muted/30">
                    ...
                    <th className="text-center px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Copies</th>
                  ...
                  {books?.map((book) => (
                    <tr key={book.id} className="border-t border-border/20 hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-5 py-3 text-center">
                        <span className="text-foreground font-medium">{book.copies}</span>
                        <span className="text-muted-foreground">/{book.totalCopies}</span>
                      </td>
                      <td className="px-5 py-3 text-center">
                        <button
                          onClick={() => handleDeleteBook(book.id, book.title)}
                          className="p-2 rounded-[--radius-sm] text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all"
